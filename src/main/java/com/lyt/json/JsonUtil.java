...
public class JsonUtil {
	...
	public static String obj2JsonString(Object obj, String[] include, boolean nory) {
		System.out.println("--------------------将java对象指定的属性转换成json字符串--------------------------");
	...
	public static <E> String list2JsonString(List<E> list) {
		System.out.println("***************将java对象List集合转换成json字符串*****************");
	...
	public static <E> String list2JsonString(List<E> beans, String[] include, boolean nory) {
		System.out.println("************将List中所含对象指定的属性转换成json字符串********************");
	...
	public static <E, V> String map2JsonString(Map<E, V> map, String[] include, boolean nory) {
		System.out.println("--------------map集合转换成json格式数据--------------");
		String s_json = "{";
	...
	public static <T> T jsonString2Object(String jsonString, Class<T> clazz) {
		System.out.println("-----------------从一个JSON 对象字符格式中得到一个java对象-----------------");
		JSONObject jsonObject = JSONObject.fromObject(jsonString);
		T bean = (T) JSONObject.toBean(jsonObject);
		return bean;
	}
	/**
    ...
    public static<E, V> Map<E, V> jsonString2Map(String jsonString) {
    	System.out.println("**********************从json HASH表达式中获取一个map，改map支持嵌套功能*********************");
    	JSONObject jsonObject = JSONObject.fromObject(jsonString);
	...
	public static <T> List<T> json2ObjectList(String jsonString, Class<T> beanClass) {
    	System.out.println("---------------------从json对象集合表达式中得到一个java对象列表--------------------");
    ...
    public static Object[] jsonToObjectArray(String jsonString) {
    	System.out.println("-----------------从json数组中得到相应java数组-----------------");
    	JSONArray jsonArray = JSONArray.fromObject(jsonString);
    ...
    public static String[] jsonToStringArray(String jsonString) {
    	System.out.println("-----------------从json数组中解析出java字符串数组------------------");
    	JSONArray jsonArray = JSONArray.fromObject(jsonString);
    ...
    public static Long[] jsonToLongArray(String jsonString) {
    	System.out.println("*********************从json数组中解析出javaLong型对象数组***********************");
    	JSONArray jsonArray = JSONArray.fromObject(jsonString);
    	int size = jsonArray.size();
    ...
    public static Integer[] jsonToIntegerArray(String jsonString) {
    	System.out.println("===============从json数组中解析出java Integer型对象数组================");
    	JSONArray jsonArray = JSONArray.fromObject(jsonString);
        int size = jsonArray.size();
    ...
    public static void main(String[] args) {
    	...
    	user.setAge(24);
    	System.out.println("一、java对象转换成json字符串：  \n" + JsonUtil.obj2JsonString(user));
    	Map<String, String> map11 = new HashMap<String, String>();
    	map11.put("1", "a");
    	map11.put("2", "b");
    	System.out.println("二、java对象转换成json字符串： \n " + JsonUtil.obj2JsonString(map11));
    	
    	System.out.println("****************************************************************");
    	List<User> list = new ArrayList<User>();
    	list.add(user);
    	list.add(user2);
    	System.out.println("三、java对象List集合转换成json字符串  \n " + JsonUtil.list2JsonString(list));
    	System.out.println("四、List集合转换成json字符串  \n " + JsonUtil.list2JsonString_(list));
    	System.out.println("****************************************************************");
        // 只包含数组里面的属性
    	System.out.println("五、java对象指定的属性转换成json字符串： \n " + JsonUtil.obj2JsonString(user, new String[]{ "username" }, true));
    	//除数组之外的所有属性
    	System.out.println("六、java对象指定的属性转换成json字符串： \n " + JsonUtil.obj2JsonString(user, new String[] { "username" }, false));
    	System.out.println("七、List中所含对象指定的属性转换成json字符串： \n " + JsonUtil.list2JsonString(list, new String[] { "username", "password" }, true));
    	
    	System.out.println("****************************************************************");
    	Map<String, User> map1 = new HashMap<String, User>();
        map1.put("1", user);
        map1.put("2", user2);
    	System.out.println("八、map集合转换成json格式数据： \n " + JsonUtil.map2JsonString(map1, new String[] { "username", "password" }, true));
    	Map<String, List<User>> map2 = new HashMap<String, List<User>>();
    	map2.put("1", list);
    	System.out.println("九、map集合转换成json格式数据： \n " + JsonUtil.map2JsonString(map2, new String[] { "username", "password" }, true));
    }
}
