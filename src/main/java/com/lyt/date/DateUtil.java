...
public class DateUtil {
	 ...
	 * @param datestr
	 * @return
	 * 利用 String replaceAll()方法  结合正则处理
	 */
	...
	public static String getFmtDateStr(String src, String infmt, String outfmt){
		...
		Date d = null;
		try {
			d = informater.parse(src);
			result = outfomater.format(d);
//			result = outfomater.format(informater.parse(src));
	...
	public static Date stringtoDate(String dateStr, String format){
		...
		try {
			...
			formater.setLenient(false);//制定日期\时间解析是否不严格
	...
	/**
	 * 把日期转换为字符串  SimpleDateFormat.format(Date)
	 */
